...
def parsing_news(request):
    ...
    while str(responce) != '<Response [404]>':
        ...
        for item in list_of_find_one_page:
            ...
            News.objects.create(text=f'{block_of_news}', title=Title.objects.create(name=f'{soup_header}'),
                                url=Url.objects.create(name=f'{url_one_news}'))
